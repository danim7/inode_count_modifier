...
void quiet_com_err_proc(const char *whoami EXT2FS_ATTR((unused)), errcode_t code EXT2FS_ATTR((unused)), const char *fmt EXT2FS_ATTR((unused)), va_list args EXT2FS_ATTR((unused)))
{
}
...
errcode_t mark_table_blocks(ext2_filsys fs, ext2fs_block_bitmap bmap)
{
	...
	for (i = 0; i < fs->group_desc_count; i++) {
		...
		if (blk)
			ext2fs_mark_block_bitmap_range2(bmap, blk, fs->inode_blocks_per_group);
	...
	if (ext2fs_has_feature_mmp(fs->super) && fs->super->s_mmp_block > fs->super->s_first_data_block && fs->super->s_mmp_block < ext2fs_blocks_count(fs->super))
