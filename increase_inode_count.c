...
errcode_t increase_inode_count(ext2_filsys fs, int flags, errcode_t(*progress) (ext2_resize_t rfs, int pass, unsigned long cur, unsigned long max_val), unsigned int new_inodes_per_group)
{
	ext2_resize_t rfs;
	errcode_t retval;
...
static void fix_uninit_block_bitmaps(ext2_filsys fs)
{
	...
	for (g = 0; g < fs->group_desc_count; g++) {
		...
		ext2fs_mark_block_bitmap2(fs->block_map, ext2fs_block_bitmap_loc(fs, g));
		ext2fs_mark_block_bitmap2(fs->block_map, ext2fs_inode_bitmap_loc(fs, g));
		for (i = 0, blk = ext2fs_inode_table_loc(fs, g); i < fs->inode_blocks_per_group; i++, blk++)
...
static errcode_t resize2fs_get_alloc_block(ext2_filsys fs, blk64_t goal EXT2FS_ATTR((unused)), blk64_t *ret)
{
...
static errcode_t block_mover(ext2_resize_t rfs)
{
	...
	int bb_modified = 0;
	ext2_filsys fs_bb_inode = get_fs_of_ino(rfs, EXT2_BAD_INO);
	...
	if (!rfs->itable_buf) {
		retval = ext2fs_get_array(fs->blocksize, fs->inode_blocks_per_group, &rfs->itable_buf);
		if (retval)
			goto errout;
	}
	 ...
	 */
	to_move = moved = 0;
	for (blk = B2C(old_fs->super->s_first_data_block); blk < ext2fs_blocks_count(old_fs->super); blk += EXT2FS_CLUSTER_RATIO(fs)) {
	...
	while (1) {
		...
		printf("Moving %llu blocks %llu->%llu\n", (unsigned long long)size, (unsigned long long)old_blk, (unsigned long long)new_blk);
		...
		do {
			...
			if (retval)
				goto errout;
			retval = io_channel_write_blk64(fs->io, new_blk, c, rfs->itable_buf);
			if (retval)
				goto errout;
 ...
 errout:
	if (badblock_list) {
		if (!retval && bb_modified)
			retval = ext2fs_update_bb_inode(fs_bb_inode, badblock_list);
		ext2fs_badblocks_list_free(badblock_list);
	}
	return retval;
}

static int update_block_reference(ext2_filsys fs, blk64_t *block_nr, e2_blkcnt_t blockcnt, blk64_t ref_block EXT2FS_ATTR((unused)), int ref_offset EXT2FS_ATTR((unused)), void *priv_data)
{
	...
	if (pb->rfs->bmap) {
		...
		if (new_block) {
			*block_nr = new_block;
			...
			printf("ino=%u, blockcnt=%lld, %llu->%llu, frees (%s): %llu\n", pb->old_ino, (long long)blockcnt, (unsigned long long)block, (unsigned long long)new_block, fs == pb->rfs->new_fs ? "new_fs" : "old_fs", ext2fs_free_blocks_count(fs->super));
...
static errcode_t migrate_ea_block(ext2_extent bmap, ext2_filsys fs, ext2_ino_t ino, struct ext2_inode *inode, int *changed)
{
	...
	if (ext2fs_file_acl_block(fs, inode) == 0 || !bmap)
		return 0;
	new_block = extent_translate(fs, bmap, ext2fs_file_acl_block(fs, inode));
	if (new_block == 0)
		return 0;
...
static errcode_t inode_scan_and_fix(ext2_resize_t rfs)
{
	...
	for (ino = 1; ino <= rfs->old_fs->super->s_inodes_count; ino++) {
		 ...
		 */
		fs->flags |= EXT2_FLAG_IGNORE_CSUM_ERRORS;
		if (ext2fs_inode_has_valid_blocks2(fs, inode) && (rfs->bmap)) {
			pb.ino = ino;
			pb.old_ino = ino;
			pb.has_extents = inode->i_flags & EXT4_EXTENTS_FL;
			retval = ext2fs_block_iterate3(fs, ino, 0, block_buf, update_block_reference, &pb);
			if (retval || pb.error)
...
static errcode_t allocate_new_itables(ext2_resize_t rfs, unsigned int *allocated_new_itables)
{
...
static errcode_t migrate_inodes_forward_loop(ext2_resize_t rfs, unsigned int *evacuated_inodes)
{
	...
	for (ino_num = 1; ino_num <= rfs->old_fs->super->s_inodes_count; ino_num++) {
		if (!ino_num) {
			/*Overflow case... rfs->old_fs->super->s_inodes_count is already MAX of ext2_ino_t
			   ...
			   Break just in case. The itable_status_populated shall have been set when treating the last inode */
			break;
		}
...
static errcode_t make_room_for_new_itables(ext2_resize_t rfs)
{
	...
	retval = ext2fs_read_bb_inode(get_fs_of_ino(rfs, EXT2_BAD_INO), &badblock_list);
	if (retval) {
...
static errcode_t inode_relocation_to_bigger_tables(ext2_resize_t rfs, unsigned int new_inodes_per_group)
{
	...
	do {
		retval = allocate_new_itables(rfs, &allocated_new_itables);
		if (retval) {
		...
		retval = migrate_inodes_forward_loop(rfs, evacuated_inodes);
		if (retval) {
		...
		if (allocated_new_itables < rfs->new_fs->group_desc_count) {
			retval = make_room_for_new_itables(rfs);
			if (retval) {
				goto errout;
			}
		}
