			 ...
			 int ref_offset EXT2FS_ATTR((unused)), void *priv_data)
{
	...
	retval = ext2fs_get_array(pb->rfs->old_fs->blocksize, 3, &block_buf);
...
/*#endif*/
	...
	if (pb->is_dir) {
		...
		retval = ext2fs_read_dir_block4(pb->rfs->old_fs, block, block_buf,0,pb->ino);
		//printf("ext2fs_read_dir_block4, block: %llu, ino: %u, retval: %li\n", block, pb->ino, retval);
		retval = ext2fs_read_dir_block4(pb->rfs->old_fs, block, block_buf,0,pb->old_ino);
		//printf("ext2fs_read_dir_block4, block: %llu, ino: %u, retval: %li\n", block, pb->old_ino, retval);
		//retval = ext2fs_write_dir_block4(pb->rfs->new_fs, block, block_buf,0,pb->ino);
		//printf("ext2fs_write_dir_block4, block: %llu, ino: %u, retval: %li\n", block, pb->ino, retval);
	}
