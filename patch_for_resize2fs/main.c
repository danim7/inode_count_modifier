...
#define _SC_PAGESIZE _SC_PAGE_SIZE
	...
	if ((flags & RESIZE_DISABLE_64BIT) && (flags & RESIZE_ENABLE_64BIT)) {
		...
		retval = calculate_new_inodes_per_group(fs, 0, new_inode_count, &new_inodes_per_group, force);
		...
		flags |= (new_inodes_per_group > fs->super->s_inodes_per_group) ? RESIZE_INCREASE_INODE_COUNT : RESIZE_DECREASE_INODE_COUNT;
