...
path_to_bin=$1
image_file=/tmp/${script_name}_tmpfs/test.low_space.ext4.img
...
$path_to_bin -f -i 196608 $image_file > ${script_name}_output_test_1 || { echo 'modification 1 failed' ; exit 1; }
...
$path_to_bin -f -i 196608 $image_file > ${script_name}_output_test_3 || { echo 'modification 3 failed' ; exit 1; }
e2fsck -vf $image_file  || { echo 'test 3 failed' ; exit 1; }

$path_to_bin -i 98304 $image_file > ${script_name}_output_test_4 || { echo 'modification 4 failed' ; exit 1; }
e2fsck -vf $image_file  || { echo 'test 4 failed' ; exit 1; }

$path_to_bin -f -i 130560 $image_file > ${script_name}_output_test_5 || { echo 'modification 5 failed' ; exit 1; }
