...
$path_to_bin -i 17408 $image_file > ${script_name}_output_test_1
if [ $? -ne 1 ]
then
...
$path_to_bin -f -i 17408 $image_file > ${script_name}_output_test_2 || { echo 'modification 2 failed' ; exit 1; }
...
cd ${mount_dir}
delete=`ls -lai /tmp/test_stable_inodes.tmpfs.sh_mounted/ |  sed -e 's/^[ \t]*//' | cut -f1 -d" " | grep -v "total" | sort -n | tail -1`
delete=$(($delete - $((count / 2))))
echo "deleting inodes below" $delete
find ${mount_dir} -inum -$delete -type f -exec rm {} \;
cd ..
HASH_A=`ls -ai ${mount_dir}/* | sha1sum | cut -f1 -d" "`
rhash -Hr ${mount_dir} > ${script_name}_SHA1SUM
used_inode_count=`df -i ${mount_dir} | tail -n +2  | tr -s " "  | cut -d" " -f3`
sudo umount ${mount_dir}
e2fsck -f $image_file
...
$path_to_bin -f -i 24576 $image_file > ${script_name}_output_test_4 || { echo 'modification 4 failed' ; exit 1; }
