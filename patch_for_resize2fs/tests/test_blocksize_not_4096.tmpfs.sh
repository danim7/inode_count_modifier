...
$path_to_bin -f -i 267280 $image_file > ${script_name}_output_test_1 || { echo 'modification 1 failed' ; exit 1; }
e2fsck -vf $image_file  || { echo 'test 1 failed' ; exit 1; }

$path_to_bin -f -i 526336 $image_file > ${script_name}_output_test_2 || { echo 'modification 2 failed' ; exit 1; }
...
$path_to_bin -f -i 263168 $image_file > ${script_name}_output_test_4 || { echo 'modification 4 failed' ; exit 1; }
e2fsck -vf $image_file  || { echo 'test 4 failed' ; exit 1; }

$path_to_bin -i 131584 $image_file > ${script_name}_output_test_5 || { echo 'modification 5 failed' ; exit 1; }
e2fsck -vf $image_file  || { echo 'test 5 failed' ; exit 1; }

$path_to_bin -i 39064 $image_file > ${script_name}_output_test_6 || { echo 'modification 6 failed' ; exit 1; }
e2fsck -vf $image_file  || { echo 'test 6 failed' ; exit 1; }

$path_to_bin -f -i 318680 $image_file > ${script_name}_output_test_7 || { echo 'modification 7 failed' ; exit 1; }
...
$path_to_bin -f -i 265720 $image_file > ${script_name}_output_test_8 || { echo 'modification 8 failed' ; exit 1; }
e2fsck -vf $image_file  || { echo 'test 8 failed' ; exit 1; }

$path_to_bin -f -i 525200 $image_file > ${script_name}_output_test_9 || { echo 'modification 9 failed' ; exit 1; }
...
$path_to_bin -i 131560 $image_file > ${script_name}_output_test_12 || { echo 'modification 12 failed' ; exit 1; }
e2fsck -vf $image_file  || { echo 'test 12 failed' ; exit 1; }

$path_to_bin -i 39000 $image_file > ${script_name}_output_test_13 || { echo 'modification 13 failed' ; exit 1; }
e2fsck -vf $image_file  || { echo 'test 13 failed' ; exit 1; }

$path_to_bin -f -i 317720 $image_file > ${script_name}_output_test_14 || { echo 'modification 14 failed' ; exit 1; }
